...
- Route evidence accepted (2026-08-20): official `coop-transition-west` v6 passed (walk-first threshold sequence, exact checkpoint contract, neutral cleanup verified). Two independent physical-keyboard `coop-route-25` runs passed all 25 alternating 140/220 transitions in 373 and 330 seconds with exact completion/pass/reconstruction/current-window pixel evidence, zero abandonment/reconstruction failures, and verified cleanup. One additional independently played route run remains. A campaign-side contract defect discovered during this run was also fixed: `postTransitionCommandedPixels` is a per-window counter that resets at each completion, so the observer now requires current pixels >=8 instead of an invalid monotonic delta; a regression test covers reset-before-pass behavior. `ce961a9`'s player-load initial-layer behavior has focused regression coverage but remains pending live verification.
- Remaining blockers: one additional 25-transition route campaign, curated contact/projectile/drop evidence, a validated read-only native EXP contract, mod-enabled native save, mod-disabled restart/load, and a human-played 60-minute soak remain. Campaigns reject automation input mutations while observing, so route runs require physical/keyboard play. M5 therefore stays In progress.
