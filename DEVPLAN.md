...
| M5 | Bounded playable local release | In progress | Three pre-fix cold/canonical passes, one post-liveness canonical pass, live telemetry/damage/revive success, and bounded reconstruction retry policy | Live-check retry pacing, then route/combat/drop/save/soak qualification |
...
- Candidate route: `NO0 9 -> 10 -> 5 -> 6 -> 5 -> 10 -> 9 -> 19 -> 11 -> 19 -> 9`. Verdict remains `candidate-untested`. Exclusions are west beyond room 6, south room 21/CEN elevator/Maria branching, bosses, water, shaped/moving terrain, and projectile-through-wall claims.
...
- Corrected route/scenario facts: telemetry predicates use `MarbleGallery`, while `NO0` is descriptive overlay code. Confirmed map cells are room 9 `32..32,26`, room 10 `29..31,26`, room 5 `26..28,26`, room 6 `22..25,26`, room 19 `33..35,26`, and room 11 `36..36,26`; these are not walkable geometry. Transition v2 uses a strict manually documented near-exit setup, P1 Left 12 rather than 240, neutral reconstruction delay, P2 Left 12 inward, and post-move/pending/failure assertions. Combat probes establish native-hit evidence after reset without impossible `EN=P` starts.
...
- Remaining blockers: live retry pacing/recovery, live room/route and 25-transition observations, three complete played route runs, curated contact/projectile/drop evidence, a validated read-only native EXP contract, mod-enabled native save, mod-disabled restart/load, and a human-played 60-minute soak remain. The visible memory-card position was empty and no native save loaded, so cold-save route/restart acceptance is not complete. M5 therefore stays In progress.
