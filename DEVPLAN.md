...
| M5 | Bounded playable local release | In progress | Three required independent 25/25 route campaigns, three pre-fix cold/canonical passes, one post-liveness canonical pass, live telemetry/damage/revive success, and bounded reconstruction retry policy | Curated combat/drop, save/reload, and soak qualification |
...
- Remaining blockers: curated contact/projectile/drop evidence, a validated read-only native EXP contract, post-retry terrain evidence, mod-enabled native save, mod-disabled restart/load, and a human-played 60-minute soak remain. M5 therefore stays In progress; no gate beyond the required route campaigns is newly claimed.
