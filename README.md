...
Native-load correctness is adapter-local and does not depend on save callbacks. VSync must first observe exactly `Game.State=MainMenu`, `g_GameStep=6`, and full-width `g_GameEngineStep=0x33`; only a later `NowLoading`/existing loading observation or the occupied-save selector progression `0x100 -> 0x101 -> 0x104` arms bootstrap. Title/MainMenu idle, unsupported selector paths (including New Game), incompatible states, fatal/unload, and a 1,800-VSync transaction timeout cancel fail closed. Ordinary Play room loading cannot arm because it never establishes the file-select latch. Once armed, the existing two consecutive post-player-update safe-sample gate, layer-accounting suppression, retained-room provisional reconstruction, stale-room rebaseline, reconstruction retry, and stable destination reconstruction closure remain authoritative. The bounded transition trace records file-select observation, loading, arm, and cancel sources with enum reasons and no save data. The complete co-op gate passes 267 focused contracts, including exact-owned lifetime, bounded retry, strict route, and data-only release-matrix validators. Current and pinned validation also execute production-shared direct-dispatch/context and scratch-restoration probes; both produced 301056-byte dynamic mod assemblies. Each compatibility surface emitted the same four existing dependency warnings: two runtime debug-panel nullable warnings, one unused runtime audio field, and the wrapper `Game.cs` nullable return. Parent automation passes 164/164, the MCP Release build is warning-free, and the full host Release build retains five previously documented warnings.
