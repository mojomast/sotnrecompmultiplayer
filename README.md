...
| --- | --- |
| Circle | One normal-engine contact window at facing offset `±14,1`, half-size `12x20`, matching the native standing body's vertical coverage. |
...
There is never more than one projectile or outgoing entity. Native attack entities are published in camera-locked screen space. Generation ordering arms the first collision for the next `HitDetection`, and normal cleanup is deferred until that observation window so the engine cannot lose the entity before collision. A projectile keeps one exact-owned slot and native `EntityNull` update while its managed world position advances before each next collision window. Each window deactivates publication fields while mutating, revalidates room/ownership/timing, clears prior hit evidence, captures bounded compatible targets, then publishes entity ID/update/hit state last. It does not pierce.
...
7. Equip a normal damaging hand item. Tap Pad 2 Circle (`O`) near an ordinary enemy, then use Up+Circle (`I+O`) from farther away. Verify the contact attack has one window, only one projectile exists, its marker moves straight, and it disappears on hit/range/timeout/transition. Confirm `X` reports the latched item tuple, extraction/restore checks, exact cleanup, valid attacker ID, and either retained hit/cooldown evidence or exact unique one-shot teardown/reward evidence. Native rewards may change.
...
The NO0 drop observer scans only pool slots `160..191` around normal engine windows. It recognizes exact prize ID 3/update `0x801C9220`, equipment ID 10/update `0x801C9C34`, and the native same-slot ID 10 to ID 3 initialization morph. A unique one-shot defeat requires exactly one pre-captured overlapping target under an exact-owned attack window, that same native slot becoming empty, a positive native EXP delta, and exactly one native kill increment. Retained hit/cooldown evidence remains the other supported defeat path. Reuse, a surviving identity, missing reward, multiple kills, capture ambiguity, and overflow fail closed. One spawn is associated only when exactly one such defeat and one matching new drop occur in the same room within 24 pixels without ambiguity. Attack-target capture stores 16 records; a seventeenth compatible target marks the window truncated, increments explicit attack-target and drop ambiguity/overflow metrics, and forbids unique defeat/drop association. Breakables, ambient/simultaneous spawns, wrong updates/rooms/positions, and pending-window exhaustion never become P2 associations. Fixed arrays and four bounded pending defeats avoid per-update allocations. Collection is generic only when pickup-step/hit evidence and a matching native reward delta both exist; without safe inventory proof, diagnostics never claim equipment collection. Native EXP samples use a read-only `uint` load from status offset `0x288`, promoted to `long` only after memory, Play, loading, Alucard, and Marble Gallery gates pass. The before/after samples span the actual native collision/reward frame; causal EXP is recorded there even if a drop appears later. Unavailable or faulting samples are `null`, every opened window completes or cancels, and observed deltas remain evidence rather than guaranteed P2 reward attribution.
...
The bounded catalog also contains setup-gated M5 probes. Game telemetry predicates use exact enum string `MarbleGallery`; `NO0` remains the overlay code in descriptions. `coop-transition-west` requires area 40 telemetry room 140/map cell `32,27` (the lower clock-room junction), with P1 manually placed on flat floor no more than eight walkable world pixels east of the west exit. Its equal 252-frame timelines first walk P1 west into the live-observed threshold step to guarantee contact, then hop it (P1 Left 60, Left+Cross 12, then Left 55), delay for reconstruction, then use P2 Left/inward 8; transition, pass, reconstruction, movement, abandonment, and failure evidence is per-run through deltas. Contact/projectile scenario v4 requires a curated `EN=P` target and exactly one current compatible target, preserves that placement with reset `none` because diagnostic reset invalidates it, then uses neutral 8, attack 2, and bounded neutral recovery with exact per-run lifecycle checks. Damage/revive retains reset `none` and requires revive deltas while avoiding unavailable `PlayerHasControl`. `coop-drop-observe` is reset-none and requires an already observed unique native-drop association; repeated curated attempts may be needed, while a causal no-drop attempt is valid but incomplete evidence. These are honest probes, not prerecorded passes or claims that their setups are qualified.
...
Castle Entrance stairs also exposed the pre-retry reconstruction churn: diagnostics alternated Suspended/Reconstructing with 206 suspension entries (maximum consecutive 1), 219 reconstruction attempts, and repeated failures. Moving Player 1 to flat terrain eventually recovered. A later unsupported-terrain live trace observed the bounded post-fix policy: one failed reconstruction, 29 suppressed attempts, one retry, then `Selected` recovery with the cooldown cleared and no fatal state. This verifies the retry cadence and recovery contract, not broad unsupported-terrain support.
...
The same session reached the native file selector, but the visible memory-card position was empty and no native save was loaded. Therefore it is not cold-save, restart/load, or save-integrity acceptance evidence. The required route evidence and bounded post-retry policy observation were completed later, but save/restart, contact/projectile/drop qualification, and the playable soak remain open.

The 2026-08-21 combat session proved that published Player 2 attacks can kill the curated target and that native EXP and kill counters advance. It also exposed and drove fixes for camera-space publication, contact vertical geometry, collision-generation cleanup ordering, and one-shot observer attribution. After the final fixes, room 52 scene instability and exact-one-target activation prevented an accepted contact, projectile, or drop artifact. The deterministic private memory card used for setup was not committed. Full validation gates are intentionally deferred to a separate run; no unfinished combat, save, or soak gate is claimed here.
...
- Structured diagnostics now observe bounded NO0 native drop lifecycle, causal association, and validated read-only native EXP deltas. An accepted associated-drop artifact, equipment inventory proof, broader overlays, and reward/progression policy remain blocked.
...
The M3/M4 baseline is complete. M5 is in progress with `p2d4/2`, protocol `1.2`, scenario v2, setup-gated probes, visible tether/suspension policy, public candidate route manifest `routes/no0-marble-gallery-candidate.json`, and parent-owned background observers `coop-route-25` and `coop-soak-60m`. The observers never launch, reset, reload, select saves, or drive input; humans prepare and play the session. The data-only `release/m5-release-matrix.json` records exact objectives and human approvals without claiming completion. The candidate route is the v2 live-observed `NO0 140 -> 220` alternating ping-pong across the flat clock junction/save-room doorway (telemetry room bytes; room table indexes 21 and 31). Three pre-liveness cold canonical passes and one post-liveness canonical pass have live evidence, the v6 transition probe passed, and three independent physical-keyboard `coop-route-25` runs accepted all 25 transitions in 373, 330, and 321.5981476 seconds with verified cleanup. The third artifact, `campaign-20260821T014103613Z-3b409ee856d06450`, records process 985721, session `62bd6e2a0422451f81ae8bf03ff07cec`, diagnostic generation 0, parent build `0.2.0+2206c03ca0e46280e244ed5c2815d99376ce402a`, and co-op `0.4.0` including native-load bootstrap through `56df1c6`; cleanup was attempted, succeeded, and verified. This completes only the required three-route evidence. The prior `ce961a9` pending-live-verification claim is superseded: the native-load bootstrap fix through `56df1c6` was verified live from file select through selected-save progression, provisional stale room 44, nonsemantic room 140 retries with `transitionPending=false` and zero reconstruction failures, and stable room 140 closure as `CHANGED_IDENTITY`; the subsequent `140 -> 220` crossing completed normally at exact completed +1/pass +1 with 9 pixels and zero abandonment or transition reconstruction failure. A patched memory card provides the progressed Marble Gallery spawn. Contact/projectile/drop evidence, post-retry terrain evidence, mod-enabled native save, mod-disabled restart/load, and a human-played 60-minute soak remain blockers.
...
Native-load correctness is adapter-local and does not depend on save callbacks. VSync must first observe exactly `Game.State=MainMenu`, `g_GameStep=6`, and full-width `g_GameEngineStep=0x33`; a later `NowLoading`/existing loading observation, the occupied-save selector progression `0x100 -> 0x101 -> 0x104`, or the direct fully normal `Play 3/1` edge arms bootstrap. The direct edge intentionally covers both occupied-save and New Game initial player loads; it requires `Game.State=Play`, game step `3`, engine step `1`, and loading false. Title/MainMenu idle, unsupported selector paths, incompatible or unsafe Play states, fatal/unload, and a 1,800-VSync transaction timeout cancel fail closed. Ordinary Play room loading cannot arm because it never establishes the file-select latch. Once armed, the existing two consecutive post-player-update safe-sample gate, layer-accounting suppression, retained-room provisional reconstruction, stale-room rebaseline, reconstruction retry, and stable destination reconstruction closure remain authoritative. The bounded transition trace records file-select observation, loading, arm, and cancel sources with enum reasons and no save data; consecutive per-frame retry suppressions are coalesced while retry metrics remain complete.
