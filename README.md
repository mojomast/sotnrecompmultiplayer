...
The native-load bootstrap retains the room identity present at `PlayerLoaded`. A selected reconstruction of that same identity remains provisional, because live engine ordering can publish stale room state after the initial layer callback; a stable selected reconstruction of a different engine-observed identity closes bootstrap. Identity churn while armed rebaselines without a semantic transition, and bounded transition trace records the post-load-identity closure reason. The complete co-op gate passes 258 focused contracts, including exact-owned lifetime, bounded retry, strict route, and data-only release-matrix validators. Current and pinned validation also execute production-shared direct-dispatch/context and scratch-restoration probes; both produced 291328-byte dynamic mod assemblies. Each compatibility surface emitted the same four existing dependency warnings: two runtime debug-panel nullable warnings, one unused runtime audio field, and the wrapper `Game.cs` nullable return. Parent automation passes 164/164, the MCP Release build is warning-free, and the full host Release build retains five previously documented warnings.
